"""Assessment Detail's own manual "Re-Assess" trigger -- previously the
...
onboarded app gets a plain submit button (no confirm dialog, matching
Fleet's plain "Re-assess"); an already-onboarded app gets a confirm dialog
first (matching Fleet's "Refresh Onboard", since re-assessing there also
auto-chains into onboard).
"""
...
class TestAssessmentDetailReassessButton:
    async def test_never_onboarded_app_gets_plain_reassess_button(self, portal_client):
        ...
        assert "title: 'Re-Assess'" not in resp.text
    ...
    async def test_onboarded_app_gets_confirm_dialog_reassess_button(self, portal_client):
        ...
        # gets a confirm dialog, matching Fleet's "Refresh Onboard".
        assert "title: 'Re-Assess'" in resp.text
