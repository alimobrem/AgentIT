...
# ── create_agent_prs — dedup against current default branch ─────────────
#
# Regression tests for the recurring redundant-PR pattern (PRs #85/#89/#90/
# #91): the codechange/cost/dependency agents regenerate deterministic
# advisory content on every run, and `create_agent_prs` used to
# unconditionally branch/commit/push/open-PR even when that exact content
# had already merged to the default branch via an earlier run. These prove
# the new dedup check (`_agent_content_unchanged`) actually prevents that.

_COST_REPORT_CONTENT = "# Cost Report\n\nEstimated monthly cost: $42\n"


def _cost_agent_results(content: str = _COST_REPORT_CONTENT) -> list[dict]:
    return [{
        "agent_name": "cost",
        "category": "cost",
        "files": [{
...
@patch.dict("os.environ", {"GITHUB_TOKEN": "ghp_test123"})
@patch("agentit.portal.github_pr.requests")
def test_create_agent_prs_skips_when_content_already_matches_default_branch(mock_requests):
    """Byte-identical generated content vs. what's already at the
    destination path on the (freshly fetched) default branch must skip the
    branch/commit/PR entirely -- no mutating call is made at all."""
    def mock_get(url, **kwargs):
        resp = MagicMock()
        resp.status_code = 200
        if url.endswith("/repos/org/my-app"):
        ...
        elif "/contents/.agentit/cost/cost-report.md" in url:
            resp.json.return_value = {
                "content": base64.b64encode(_COST_REPORT_CONTENT.encode()).decode(),
            }
        return resp

    mock_requests.get.side_effect = mock_get

    results = create_agent_prs(
        repo_url="https://github.com/org/my-app.git",
        repo_name="my-app",
        agent_results=_cost_agent_results(),
    )
...
@patch.dict("os.environ", {"GITHUB_TOKEN": "ghp_test123"})
@patch("agentit.portal.github_pr.requests")
def test_create_agent_prs_opens_pr_when_content_differs(mock_requests):
    ...
    def mock_post(url, **kwargs):
        ...
        if "git/trees" in url:
            ...
            resp.json.return_value = {"ref": "refs/heads/agentit/cost"}
        elif "/pulls" in url:
            resp.json.return_value = {"html_url": "https://github.com/org/my-app/pull/90"}
        return resp
    ...
    assert results == [{
        "agent_name": "cost",
        "branch": "agentit/cost",
        "pr_url": "https://github.com/org/my-app/pull/90",
        "files_count": 1,
    }]
...
@patch.dict("os.environ", {"GITHUB_TOKEN": "ghp_test123"})
@patch("agentit.portal.github_pr.requests")
def test_create_agent_prs_second_identical_run_noops_after_first_merges(mock_requests):
    """The exact real-world sequence that produced PRs #85/#89/#90/#91: run 1
    generates+commits+opens a PR for genuinely new content; that PR merges
    to the default branch (which also advances to a new commit sha,
    proving the dedup check reads a live, current ref rather than a
    cached/stale one); run 2 regenerates the *same deterministic content*
    and must no-op instead of opening a second, redundant PR."""
    ...
    def mock_get(url, **kwargs):
        ...
        if url.endswith("/repos/org/my-app"):
            ...
            if main_state["content"] is None:
                ...
                resp.status_code = 200
                resp.json.return_value = {
                    "content": base64.b64encode(main_state["content"].encode()).decode(),
                }
        return resp
    ...
    def mock_post(url, **kwargs):
        ...
        if "git/trees" in url:
            ...
            resp.json.return_value = {"html_url": "https://github.com/org/my-app/pull/85"}
        return resp

    mock_requests.get.side_effect = mock_get
    mock_requests.post.side_effect = mock_post

    # Run 1: nothing on main yet -- opens a real PR.
    run_1 = create_agent_prs(
        repo_url="https://github.com/org/my-app.git",
        repo_name="my-app",
        agent_results=_cost_agent_results(),
    )
    assert run_1 == [{
        "agent_name": "cost",
        "branch": "agentit/cost",
        "pr_url": "https://github.com/org/my-app/pull/85",
        "files_count": 1,
    }]
    assert mock_requests.post.call_count >= 1

    # The PR from run 1 merges -- main now has the generated content and a
    # new HEAD sha (a fresh ref, not the one fetched during run 1).
    main_state["content"] = _COST_REPORT_CONTENT
    main_state["sha"] = "sha-main-2"
    mock_requests.post.reset_mock()

    # Run 2: the agent regenerates byte-identical content -- must no-op.
    run_2 = create_agent_prs(
        repo_url="https://github.com/org/my-app.git",
        repo_name="my-app",
        agent_results=_cost_agent_results(),
    )
...
@patch.dict("os.environ", {"GITHUB_TOKEN": "ghp_test123"})
@patch("agentit.portal.github_pr.requests")
def test_create_agent_prs_fetches_default_branch_ref_fresh_every_call(mock_requests):
    """`create_agent_prs` must re-fetch `git/ref/heads/{default_branch}` on
    every invocation rather than reusing a sha cached from an earlier call
    -- each of two calls here reports a different current main sha, and
    both calls must be observed making their own live GET for it (no
    module-level caching of a stale ref)."""
    call_shas = iter(["sha-main-1", "sha-main-2"])
    ...
    def mock_get(url, **kwargs):
        ...
        if url.endswith("/repos/org/my-app"):
            ...
            resp.json.return_value = {"object": {"sha": next(call_shas)}}
        elif "/contents/.agentit/cost/cost-report.md" in url:
            resp.json.return_value = {
                "content": base64.b64encode(_COST_REPORT_CONTENT.encode()).decode(),
            }
    ...
    ref_calls = [c for c in mock_requests.get.call_args_list if "git/ref/heads/main" in str(c)]
