...
class TestAutoValidateAndDeliver:
    ...
    async def test_self_managed_chart_gate_refused_is_needs_attention(self):
        ...
        raw_skill_dump = {
            ...
            # A real, registered SOLUTION_CONTRACT category (FIX_REGISTRY's
            # "availability" -> pdb) -- not the generic "test" placeholder
            # other tests in this file use, so Phase A's finding gate
            # (contracted-category check) doesn't refuse this batch before
            # it ever reaches the self-managed chart gate this test targets.
            "finding_addressed": "availability",
            "skill_name": "pdb",
        }
        ...
        with patch("agentit.portal.github_pr.create_source_patch_pr") as mock_source, \
            ...
            result = await auto_validate_and_deliver(
                ...
                orchestration={},
                target_findings=[("availability", "missing PDB")],
            )
...
class TestPerClusterIsolation:
    """Regression, confirmed live (pulse-agent, 2026-07-24): validation used
    to run once over the whole undifferentiated ``cluster_layer`` batch,
    sharing ONE clean/fail verdict across every unrelated finding in the
    same onboarding. An unrelated, unfixable finding (there: "Autoscaling",
    with no candidate file at all) silently discarded a completely
    unrelated, individually-valid file (``pulse-agent-argocd-application
    .yaml``) purely because they shared a batch, with no event or error
    recorded anywhere.

    The real, root-cause fix: partition into per-finding clusters BEFORE
    any validate/fix work runs, then validate/fix/retry EACH cluster in
    isolation -- one cluster's failure can never affect a sibling's
    outcome, and every refusal is a real, visible store event. These
    tests prove that isolation property directly with three independent
    clusters in one batch (one broken-and-unfixable, one clean, one
    source-patch), not just the one reproduction case that happened to
    bite production.
    """

    async def test_one_broken_cluster_never_blocks_a_clean_sibling_or_a_source_patch(self):
        store = await make_store()
        report = _report_with_finding(
            "rbac", repo_name="isolation-app",
            infra_repo_url="https://github.com/org/isolation-app-gitops",
        )
        report.scores[0].findings.append(Finding(
            category="gitops", severity=Severity.high,
            description="missing gitops registration", recommendation="add gitops",
        ))
        report.scores[0].findings.append(Finding(
            category="container", severity=Severity.high,
            description="missing container pin", recommendation="pin container",
        ))
        aid = await store.save(report)

        # Cluster 1: RBAC -- passes clear-evidence's coarse "some RBAC kind
        # exists" check (a ServiceAccount alone qualifies) but is actually
        # incomplete (no Role, no RoleBinding) -- property_verifier's RBAC
        # check correctly fails it, and the bogus dispatcher fix below
        # (same shape as TestValidateAndFixManifests::
        # test_gives_up_honestly_after_max_iterations_when_fix_never_
        # resolves above) never actually completes it either. Must end in
        # an isolated, visible refusal, never a crash or silent drop.
        broken_rbac_file = {
            "category": "skills",
            "path": "isolation-app-rbac.yaml",
            "content": "apiVersion: v1\nkind: ServiceAccount\nmetadata:\n  name: isolation-app\n",
            "description": "RBAC",
            "finding_addressed": "rbac",
            "skill_name": "rbac",
        }
        # Cluster 2: gitops -- individually clean and unrelated to RBAC; a
        # real, clear-evidence-satisfying Application manifest standing in
        # for the actual pulse-agent-argocd-application.yaml. Must survive
        # and deliver regardless of cluster 1's fate.
        clean_gitops_file = {
            "category": "skills",
            "path": "isolation-app-argocd-application.yaml",
            "content": (
                "apiVersion: argoproj.io/v1alpha1\nkind: Application\n"
                "metadata:\n  name: isolation-app\n"
                "spec:\n  source:\n"
                "    repoURL: https://github.com/org/isolation-app.git\n"
                "    path: chart/\n"
            ),
            "description": "Generated by skill argocd-application",
            "finding_addressed": "gitops",
            "skill_name": "argocd-application",
        }
        # Cluster 3: source-layer container patch -- never SSA-validated,
        # must deliver independent of both cluster-layer outcomes.
        source_file = {
            "category": "codechange",
            ...
            # The real registered SOLUTION_CONTRACT skill for "container"
            # is "containerfile" (see registry.py) -- filter_files_to_open_
            # findings()'s solution-contract matching only accepts the
            # actual registered clearing skill, not the finding category
            # name itself.
        ...
        bogus_fix = {
            "files": [{
                "category": "security", "path": "isolation-app-rbac.yaml",
                "content": "apiVersion: v1\nkind: ConfigMap\nmetadata:\n  name: still-not-rbac\n",
                "description": "still does not satisfy the RBAC property",
            }],
            "agent": "security", "method": "rbac", "error": None,
        }

        def _fake_commit(infra_repo_url, app_name, files, *args, **kwargs):
            return {
                "pr_url": f"{infra_repo_url}/pull/1",
                "commit_url": f"{infra_repo_url}/commit/abc",
                "files_committed": len(files),
            }

        with patch("agentit.portal.delivery.kube.get_custom_resource", return_value=None), \
             patch(
                 "agentit.remediation.dispatcher.RemediationDispatcher.dispatch",
                 return_value=bogus_fix,
             ), \
             patch(
                 "agentit.portal.github_pr.create_source_patch_pr",
                 return_value={
                     "pr_url": "https://github.com/org/isolation-app/pull/1",
                     "files_committed": 1,
                 },
             ), \
            ...
            result = await auto_validate_and_deliver(
                ...
                files=[broken_rbac_file, clean_gitops_file, source_file],
                orchestration={},
                target_findings=[
                    ("rbac", "missing rbac"),
                    ("gitops", "missing gitops registration"),
                    ("container", "missing container pin"),
                ],
            )

        # The two healthy clusters delivered -- the broken one never
        # blocked either of them.
        assert result["status"] == "delivered"
        assert len(result["pr_urls"]) == 2

        # The broken cluster's own refusal is a real, visible event -- not
        # just a Python logger line nobody but an operator could ever see.
        events = await store.list_events_by_correlation_id(aid)
        rbac_refusal_events = [
            e for e in events
            if e["agent_id"] == "auto-delivery" and "rbac" in e["summary"].lower()
            and e["severity"] == "warning"
        ]
        assert rbac_refusal_events, (
            "rbac cluster's refusal must be a real, visible store event -- "
            f"got events: {[(e['action'], e['summary']) for e in events]}"
        )

        # The gitops file (and the source patch) survived into the final
        # saved onboarding batch -- the exact property the live pulse-agent
        # bug violated (an unrelated failure silently erasing this file).
        saved = await store.get_onboarding(aid)
        saved_paths = {f["path"] for f in saved}
        assert clean_gitops_file["path"] in saved_paths
        assert source_file["path"] in saved_paths
