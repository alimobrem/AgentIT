...
def test_skips_non_yaml_files():
    with patch("agentit.portal.cluster_apply._find_cli", return_value="oc"):
...
def test_applies_yml_files():
    mock_result = MagicMock()
    mock_result.returncode = 0
    mock_result.stdout = "created"
    with patch("agentit.portal.cluster_apply._find_cli", return_value="kubectl"):
        with patch("subprocess.run", return_value=mock_result):
            result = apply_manifests_to_cluster([_yaml_file("dependabot.yml")], dry_run=True)
            assert result["applied"] == ["dependabot.yml"]
