...
def edit_client():
    ...
    store.save_onboarding(assessment_id, [_configmap_file()])
...
class TestStoreUpdateOnboardingFile:
    def test_persists_content_and_captures_original(self):
        store = make_store()
        report = make_report()
        aid = store.save(report)
        store.save_onboarding(aid, [_configmap_file()])

        updated = store.update_onboarding_file(aid, "skills", "app-config.yaml", "edited: true\n")
    ...
    def test_original_content_preserved_across_multiple_edits(self):
        store = make_store()
        report = make_report()
        aid = store.save(report)
        store.save_onboarding(aid, [_configmap_file()])

        store.update_onboarding_file(aid, "skills", "app-config.yaml", "first edit\n")
        second = store.update_onboarding_file(aid, "skills", "app-config.yaml", "second edit\n")
    ...
    def test_returns_none_for_unknown_path(self):
        ...
        aid = store.save(report)
        store.save_onboarding(aid, [_configmap_file()])
        assert store.update_onboarding_file(aid, "skills", "nonexistent.yaml", "x") is None

    def test_returns_none_when_no_onboarding_exists(self):
        store = make_store()
        report = make_report()
        aid = store.save(report)
        assert store.update_onboarding_file(aid, "skills", "app-config.yaml", "x") is None
...
class TestEditOnboardingFileRoute:
    def test_valid_edit_saves_and_redirects_with_success(self, edit_client):
    ...
    def test_edit_logs_an_event_with_correlation_id(self, edit_client):
        client, store, aid = edit_client
        client.post(
            f"/assessments/{aid}/onboard-results/edit-file",
            data={
    ...
    def test_invalid_yaml_edit_is_rejected_and_not_persisted(self, edit_client):
    ...
    def test_structurally_invalid_manifest_missing_metadata_is_rejected(self, edit_client):
        client, store, aid = edit_client
    ...
    def test_unknown_file_path_returns_404(self, edit_client):
    ...
    def test_unknown_assessment_returns_404(self, edit_client):
...
class TestOnboardResultsPageRendersEditAndDiff:
    def test_page_shows_editor_controls(self, edit_client):
        client, _store, aid = edit_client
    ...
    def test_page_shows_diff_after_edit(self, edit_client):
        client, _store, aid = edit_client
        new_content = "apiVersion: v1\nkind: ConfigMap\nmetadata:\n  name: test\ndata:\n  key: changed\n"
        client.post(
            f"/assessments/{aid}/onboard-results/edit-file",
    ...
    def test_no_inline_styles_introduced_by_editor_or_diff(self, edit_client):
        client, _store, aid = edit_client
        client.post(
            f"/assessments/{aid}/onboard-results/edit-file",
            data={
...
class TestDeliverUsesEditedContent:
    def test_deliver_applies_edited_content_not_original(self, edit_client, _mock_kube):
        client, store, aid = edit_client
        edited_content = (
            "apiVersion: v1\nkind: ConfigMap\nmetadata:\n  name: test\ndata:\n  key: EDITED_VALUE\n"
        )
    ...
    def test_delivery_row_records_edited_files(self, edit_client, _mock_kube):
        client, store, aid = edit_client
        client.post(
            f"/assessments/{aid}/onboard-results/edit-file",
            data={
        ...
        client.post(f"/assessments/{aid}/deliver", data={"dry_run": "false"}, follow_redirects=False)

        deliveries = store.list_deliveries(aid)
        assert len(deliveries) == 1
        assert deliveries[0]["details"]["edited_files"] == ["app-config.yaml"]

    def test_delivery_row_has_no_edited_files_when_nothing_was_edited(self, edit_client, _mock_kube):
        client, store, aid = edit_client
        client.post(f"/assessments/{aid}/deliver", data={"dry_run": "false"}, follow_redirects=False)
        deliveries = store.list_deliveries(aid)
...
class TestEditCannotBypassSafetyRouting:
    def test_editing_configmap_into_secret_blocks_delivery(self, edit_client, _mock_kube):
    ...
    def test_editing_manifest_still_goes_through_route_and_deliver(self, edit_client, _mock_kube):
