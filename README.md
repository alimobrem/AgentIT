...
- **Clear-evidence gate** — refuses theater stubs, root-only `audit.py` without package wire-up, destructive Containerfile rewrites, SLSA/hermetic prose without a real `cosign sign` Task (`image_signing` → `cosign-sign-task`), Tekton `sbom-task` theater for the app-repo `sbom` finding (`sbom` → `sbom-artifact`), and CycloneDX shells with empty `components: []` (SBOM must inventory from Syft or lockfiles/manifests)
