...
| **Gate** | Finding-tied PRs only (Phase A `finding_gate` on auto delivery **and** manual `/deliver` — no catalog dumps); SSA dry-run + clear-evidence simulation (refuses theater stubs / destructive Containerfile rewrites; root-only `audit.py` without package wire-up; hand-rolled store DDL passes `migration`). App `Containerfile` base is digest-pinned (`ubi9/python-312@sha256:…`), not floating `:latest`/`:1`. |
