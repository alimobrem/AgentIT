...
| **Ledger** | Read-only, chronological union of events, gates, deliveries, and fix-review decisions (`ledger.py::get_ledger_cards()`) into the 16 real card types in [`docs/ledger-design-spec.md`](docs/ledger-design-spec.md) — fleet-wide at `/ledger` (grouped by app by default, with a "Needs You" filter), or scoped to one app via Assessment Detail's own **Ledger** tab. A "Replay this chain" scrubber (`/ledger/chain/<correlation_id>`) plays back any correlation-id chain read-only. Additive alongside Events/Decisions/Actions/Timeline, nothing existing removed or changed. |
