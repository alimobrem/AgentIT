...
| **Learn** | Watchers surface drift/CVEs/SLOs; learning drafts skills for human activation. Decisions (`fix-review`) attributes approve/reject to the finding’s contract skill only — not every companion skill on the assessment. Decisions (`secret-classify`) logs first sight / outcome flips only — `__PLACEHOLDER__` and Prometheus secret-*name* labels never hit the LLM; repeat Scans reuse `secret_classify_cache`. |
