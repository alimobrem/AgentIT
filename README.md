...
6. **L4 outcome feedback.** Each cycle starts by polling prior self-improve PR URLs via `get_pr_status` and logging durable `capability-outcome` events (`merged` / `closed` / `stale`, with optional `agentit:reject-reason:…` from labels or PR body). The next `gather_evidence()` prefers untried doc gaps, deprioritizes recent `wontfix` titles, skips already-merged modules (e.g. `#20` stack-signature, `#23` tick-failure classifier), and cites `cited_merges` in the run's details JSON. Close a PR with label `agentit:reject-reason:wontfix` (or the same line in the body) to keep scout off that gap for 30 days.
