...
class AssessmentStore:
    ...
    async def get_skill_effectiveness(self, skill_name: str = '', min_count: int = 5) -> dict:
        if skill_name:
            rows = await self._pool.fetch(
                'SELECT skill_name, outcome, COUNT(*) as cnt FROM skill_effectiveness WHERE skill_name = $1 GROUP BY skill_name, outcome',
                skill_name,
            )
        else:
            rows = await self._pool.fetch(
