...
from agentit.portal.deploy_status import (
    _DEPLOY_STATUS_DEADLINE,
    _deploy_status_executor,
    _degraded_deploy_status,
    _get_deploy_status,
    _get_deploy_status_bounded,
    _get_last_good_deploy_status,
)
...
# URL, keeps this bounded the same way ``deploy_status.py``'s own
# ``_deploy_status_cache`` already bounds its kube-apiserver calls -- same
# threading.Lock reasoning (this runs in a real OS thread via
# `asyncio.to_thread`, so an `asyncio.Lock` wouldn't exclude concurrent
# pollers).
