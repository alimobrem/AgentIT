    ...
    if lang in ("javascript", "typescript", "node"):
        files.append(GeneratedFile(
            ...
            content=textwrap.dedent("""\
                ...
                -- prisma migrate / knex). Replace SELECT 1 with real DDL.
                SELECT 1;
            """),
        ...
        files.append(GeneratedFile(
            ...
            content=textwrap.dedent("""\
                -- +migrate Up
                -- Baseline revision for golang-migrate. Replace with real DDL.
                SELECT 1;
            """),
        ...
        files.append(GeneratedFile(
            ...
            content=textwrap.dedent('''\
                """Baseline schema revision — replace upgrade() body with real DDL."""
                ...
                def upgrade() -> None:
                    # Example:
                    # op.execute("CREATE TABLE IF NOT EXISTS example (id TEXT PRIMARY KEY)")
                    op.execute("-- agentit baseline: add real DDL in a follow-up revision")
...
def _sbom_artifact_patch(skill: "Skill", report: AssessmentReport) -> list[GeneratedFile]:
    ...
    return [GeneratedFile(
        ...
        description=(
            ...
            "(clears compliance sbom finding)"
        ),
        finding_addressed=addressed,
        skill_name=skill.name,
