...
_APPLICATION_REPO_URL_RE = re.compile(r'(?m)^([ \t]*repoURL:\s*)["\']?([^\s"\']*)["\']?\s*$')


def _force_real_repo_url_for_application_output(
    files: list[GeneratedFile], report: AssessmentReport,
) -> list[GeneratedFile]:
    """An Argo CD ``Application``'s ``repoURL`` is never a value an LLM
    should be trusted to invent -- it is 100% deterministic
    (``report.repo_url``, the same real value the template-fallback path
    already uses via ``_template_variables()``). Confirmed live: the
    ``argocd-application`` skill's LLM-tailored path fabricated
    ``https://github.com/org/pulse-ui.git`` instead of the real
    ``https://github.com/PulseSRE/pulse-ui`` -- a shape generic enough
    that ``verify_argocd_application()``'s placeholder denylist doesn't
    (and safely can't, since this codebase's own fixtures legitimately
    use that ``github.com/org/...`` shape) catch it.

    Rewrites ``repoURL`` on every generated ``kind: Application`` document
    to the real repo URL after generation, regardless of whether the
    content came from the LLM or the deterministic template path, so it
    can never silently drift to a fabricated org/repo again.
    """
    real_url = (report.repo_url or "").strip()
    if not real_url:
        return files
    ...
    for gen_file in files:
        ...
        if re.search(r"(?m)^\s*kind:\s*Application\s*$", content):
            match = _APPLICATION_REPO_URL_RE.search(content)
            if match and match.group(2) != real_url:
                logger.warning(
                    "Skill %s emitted repoURL %r for %s -- overriding with the "
                    "real repo URL %r before delivery",
                    gen_file.skill_name, match.group(2), gen_file.path, real_url,
                )
                content = _APPLICATION_REPO_URL_RE.sub(
                    lambda m: f'{m.group(1)}"{real_url}"', content, count=1,
                )
                gen_file = gen_file.model_copy(update={"content": content})
...
class SkillEngine:
    ...
    def generate(self, skill: Skill, report: AssessmentReport,
        ...
        # Fleet HPA: discover live Deployments/Rollouts and emit a correct
        # scaleTargetRef (or skip). Never invent Deployment/{app_name}.
        if (
            not self.self_managed
            and "HorizontalPodAutoscaler" in skill.outputs
        ):
            fleet_hpa = self._generate_fleet_hpa(skill, report, app_name)
            if fleet_hpa is not None:
                return fleet_hpa
            # Discovery found no safe target — fail closed (no template dump).
            logger.info(
                "Skipping skill %s for fleet app %s — no verified live "
                "scale target (Deployment/Rollout); refusing invented HPA",
                skill.name, app_name,
            )
            return []
        ...
        if llm_client and hasattr(llm_client, '_chat'):
            ...
            if llm_result:
                return _force_real_repo_url_for_application_output(llm_result, report)
        ...
        if skill.mode == "template" or skill.mode == "llm":
            ...
            if template_text:
                ...
                if self.self_managed and not self._is_self_managed_safe_content(rendered):
                    ...
                    return []
                return _force_real_repo_url_for_application_output([GeneratedFile(
    ...
    def _generate_fleet_hpa(
        self, skill: Skill, report: AssessmentReport, app_name: str,
    ) -> list[GeneratedFile] | None:
        """Build fleet HPA from live workloads, or ``None`` if none/unverifiable.

        ``None`` means the caller should refuse (empty generation) rather than
        fall through to the Deployment/{{app_name}} template.
        """
        ...
        namespace = (
            getattr(report, "namespace", None)
            or app_name
        )
        workloads = discover_namespace_workloads(str(namespace))
        if not workloads.discovery_ok:
            return None
        min_replicas = 2 if (report.criticality or "").lower() in ("high", "critical") else 1
        content = generate_fleet_hpa_yaml(
            app_name, workloads, min_replicas=min_replicas,
        )
        if not content:
            return None
        targets = workloads.preferred_scale_targets(app_name)
        target_desc = ", ".join(f"{t.kind}/{t.name}" for t in targets)
        return [GeneratedFile(
            path=f"{app_name}-{skill.name}.yaml",
            content=content,
            description=(
                f"Generated by skill {skill.name} "
                f"(scaleTargetRef from live workloads: {target_desc})"
            ),
            finding_addressed=skill.property_description,
            skill_name=skill.name,
        )]
