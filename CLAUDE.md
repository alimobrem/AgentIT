...
- **Postgres is the only supported store — not a backend option, not a flag.** `src/agentit/portal/store.py`'s `AssessmentStore` is the one and only store, fully async (`asyncpg`). There is no more SQLite code path, `store_pg.py`/`store_factory.py` (both deleted), or `AGENTIT_DB_BACKEND`/`AGENTIT_DB_PATH` env var — see `docs/postgres-migration-plan.md` for the full history of how this cutover happened (that doc is now marked historically-complete/superseded).
- **`portal/helpers.py::get_store()` is `async def`** — every caller (CLI, watchers, portal routes) gets the store via `await get_store()` (portal) or `await store.create_store(...)` (CLI/watchers), never `AssessmentStore()` directly except inside `store.py` itself. Every store method call is `await`ed.
- **`FleetOrchestrator`, `AutoMode`, `RemediationDispatcher`, and `RemediationLoop` are genuinely `async def` throughout.** Construct them with the store directly (`await get_store()`'s return value) and `await` every call into them; every internal `self._store.method(...)` call inside these four classes is itself `await`ed. **No `.raw`/`asyncio.to_thread` bridge for these four** — that idiom is only for genuinely-synchronous consumers (background assessment threads, `EventConsumer`, each watcher's own tick body, metrics/health/fleet-enrichment helpers), and even those are now bridged via `asyncio.run_coroutine_threadsafe(coro, loop)` (the store itself has no sync escape hatch at all — `AssessmentStore` has no `.raw`).
