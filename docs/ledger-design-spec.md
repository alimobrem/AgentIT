...
| **Decisions** — per-row decision log | **Absorbed** as card types C and G, with reasoning inline (fixed: `secret-classify` decisions were missing from card C's mapping and silently dropped from the Ledger until this bug was found and closed -- see `_EVENT_ACTION_TO_CARD_TYPE` in `ledger.py`). | This page's entire purpose (surface the LLM's actual reasoning next to the outcome it gated) is what card C/G *are*. |
